            ...
            <div class="px-4 whitespace-nowrap relative">
                <div class="inline-flex rounded-md" role="group">
                    <button type="button" wire:click="setRoleCustomer({{ $user->id }})"
                        class="px-4 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-r-lg hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 {{ !$user->isVendor() ? 'ring-2 ring-blue-700 text-blue-700 ' : '' }}">
                        @if (!$user->isVendor())
                            <svg wire:target="setRoleCustomer({{ $user->id }})" wire:loading.class.remove="hidden" aria-hidden="true"
                                class="hidden inline ml-2 w-6 h-6 text-gray-900 animate-spin  fill-gray-900"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" wire:target="setRoleCustomer({{ $user->id }})" wire:loading.class.add="hidden"  class="w-4 h-4 inline ml-2 fill-current"
                                viewBox="0 0 24 24">
                                <path
                    ...
                    <button type="button" wire:click="setRoleVendor({{ $user->id }})"
                        class="px-4 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-l-md hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700 {{ $user->isVendor() ? 'ring-2 ring-blue-700 text-blue-700 ' : '' }}">
                        @if ($user->isVendor())
                            <svg wire:target="setRoleVendor({{ $user->id }})" wire:loading.class.remove="hidden" aria-hidden="true"
                                class="hidden inline ml-2 w-6 h-6 text-gray-900 animate-spin  fill-gray-900"
                                viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" wire:target="setRoleVendor({{ $user->id }})" wire:loading.class.add="hidden" class="w-4 h-4 inline ml-2 fill-current"
                                viewBox="0 0 24 24">
                                <path
                                    d="M10.0007 15.1709L19.1931 5.97852L20.6073 7.39273L10.0007 17.9993L3.63672 11.6354L5.05093 10.2212L10.0007 15.1709Z">
                                </path>
                ...
                <div class="text-right w-52 truncate">
                    ...
                    <div class="group cursor-pointer inline-block border-b border-gray-400 text-center">
                        ...
                        <div
                            ...
                            <svg class="absolute text-black h-2 w-full right-0 top-full" x="0px" y="0px"
