    ...
    <x-dashboard.main title="ویرایش دسته بندی">
        ...
        <x-dashboard.card title="{{ $category->name }}">
            <form action="{{ route('dashboard.categories.update', $category) }}"
                ...
                @method('PUT')
                <div class="mb-6">
                    <label for="owner_id" class="block mb-2 text-sm font-medium text-gray-900">
                        فروشنده شهروند
                    </label>
                    <select id="owner_id" name="owner_id"
                        ...
                        @foreach ($users as $user)
                            <option value="{{ $user->id }}" {{ $category->owner_id == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label class="relative inline-flex justify-center items-center mr-5 cursor-pointer">
                        <input type="checkbox" value="1" class="sr-only peer" name="vendor" checked>
                        <div
