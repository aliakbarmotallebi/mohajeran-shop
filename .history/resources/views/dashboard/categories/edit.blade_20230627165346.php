...
<x-dashboard.main title="ویرایش دسته بندی">
    ...
    <x-dashboard.card title="{{ $category->name }}">
        <form action="{{ route('dashboard.categories.update', $category) }}" class="grid grid-cols-1 md:grid-cols-2 gap-2 p-5" enctype="multipart/form-data" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-6">
                ...
                <select id="countries" name="vendor" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
            ...
            <div class="mb-6">
                ...
                <input name="time" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" type="text" value="{{ $category->time }}">
            </div>
