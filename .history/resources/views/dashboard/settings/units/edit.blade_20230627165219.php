...
<x-dashboard.main title="ویرایش واحد">
    ...
    <x-dashboard.card title="{{ $unit->name }}">
        <form action="{{ route('dashboard.units.update', $unit) }}" class="grid grid-cols-1 md:grid-cols-2 gap-2 p-5" enctype="multipart/form-data" method="POST">
            @csrf
            @method('PUT')
            ...
            <div>
            <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center w-auto">
                ذخیره تغییرات
            </button>
        </div>
        </form>
