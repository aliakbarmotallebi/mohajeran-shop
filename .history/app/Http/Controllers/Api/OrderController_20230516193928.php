...
*              type="array",
     ...
     * )
     */
    public function paymentTheOrder(Order $order)
    {
    ...
    {
        if($this->cart->getAmountWithTaxRate() > $this->cart->user->balance())
        {
            $this->toast('موجودی کاربر کافی نمی باشد');
            return false;
        }
        
        if($this->cart->items()->incorrectExist())
        {
            $this->toast('درلیست سفارش تایید نشده وجود دارد');
            return false;
        }

        $this->createOrderTransforAs($this->cart);

       $this->toast('باموفقیت پیش سفارش ثبت شد وگارانتی صادر شد', 'success');
    ...
    protected function createWithdrawForOrder(Order $order)
    {
        ...
        try {
            ...
            TransactionWalletManager::withdraw(
                amount: $order->getAmountWithTaxRate(),
                user: $order->user,
            );
